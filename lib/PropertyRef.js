...
class PropertyRef {
  ...
  _parse(str, builder) {
    ...
    // components.versions.localisedData.id
    if (parts.length === 1) {
      ...
      try {
        _.each(parts, (part) => {
          this.relations.push(builder._modelClass.getRelation(part));
        })
      } catch (err) {
        utils.throwError('PropertyRef: unknown relation "' + parts + '"');
      }
      _.each( this.relations, (rel) => {

      });
      this.modelClasses = this.relations.relatedModelClass;
    } 
    else {
      utils.throwError('PropertyRef: no relations specified');
    }
