 ...
 *
 * Given a query parameter `someProp:eq=10` the `eq` part is the filter. The text
 * (in this case 'eq') is mapped to a function that performs the filtering.
 *
 * Filter functions take in the `PropertyRef` instance of the property to be filtered,
 ...
 * The following example you can fetch relatins eagerly by giving a `withRelated=[pets, movies]`
...
FindQueryBuilder.prototype._buildJoins = function (params, builder) {
  ...
  _.each(_.unique(relationsToJoin, 'name'), function (relation) {
    relation.join(builder, 'leftJoin');
  });
    ...
    if (orderType && orderType.indexOf('orderBy') !== -1) {
      ...
      if (propertyRef.relation) {
        if (!utils.isOneToOneRelation(propertyRef.relation)) {
          utils.throwError("Can only order by model's own properties and by OneToOne relations' properties");
        }
